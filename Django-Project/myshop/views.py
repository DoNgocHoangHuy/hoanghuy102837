...
def product(request):
    ...
    # Lọc sản phẩm theo danh mục nếu có tham số category
    category = request.GET.get('category')
    ...
    # Lấy danh sách các danh mục duy nhất để hiển thị trong template
...
def buy_now(request, product_id):
    if request.user.is_authenticated:
        ...
        return redirect('oder')  # Chuyển hướng đến trang đặt hàng
    else:
        messages.error(request, "Vui lòng đăng nhập để mua sản phẩm.")
        return redirect('/login')
# Thanh Toán
def oder(request):
    customer = Customer.objects.get(user=request.user)  # Lấy thông tin khách hàng dựa trên người dùng hiện tại
